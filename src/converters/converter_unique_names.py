...
class ConverterV1(Converter):
    ...
    def parse_load_command(self, command):
        ...
        if command['producesDataframe']:
            ...
            for data_frame in command["producesDataframe"]:
                ...
                self.graph.add((out_port_id, self.id_manager.sdtl_namespace.dataframeName, rdflib.Literal(data_frame["dataframeName"])))
                # Connect the rdf:Bag to it
                #self.graph.add((out_port_id, rdflib.URIRef(f'http://www.w3.org/1999/02/22-rdf-syntax-ns#_{item_count}'), data_frame_id))
                item_count+=1
    ...
    def parse_compute_command(self, command):
        if command["expression"]:
            if command["expression"]["$type"] == "FunctionCallExpression":
                self.parse_function_call_command(command)


    def parse_function_call_command(self, command):
        ...
        if command['consumesDataframe']:
            item_count = 1
            for data_frame in command["consumesDataframe"]:
                in_port_id = self.dataframes[data_frame["dataframeName"]]
        ...
        if command["variable"]:
            ...
            for key in command["variable"].keys():
                source_info = f'sdtl#{key}'
                self.graph.add(
                    (variable_id, rdflib.URIRef(source_info), rdflib.Literal(command["variable"][key])))

        if command["expression"]:
            if command["expression"]["$type"] == "FunctionCallExpression":
                sub_program_id = self.id_manager.get_id("Program")
                self.graph.add((sub_program_id, rdflib.RDF.type, self.id_manager.provone_ns.Program))
                # Connect the parent program to it
                self.graph.add((program_id, self.id_manager.provone_ns.hasSubProgram, sub_program_id))
                for key in command["expression"].keys():
                    if key == "arguments":
                        for arg in command["expression"][key]:
                            # Create the port
                            new_inport_id = self.id_manager.get_id("Port")
                            self.graph.add((new_inport_id, rdflib.RDF.type, self.id_manager.provone_ns.Port))
                            self.graph.add((new_inport_id, rdflib.RDF.type, self.id_manager.sdtl_namespace.functionArgument))
                            # Connect it to the program
                            self.graph.add((sub_program_id, self.id_manager.provone_ns.hasInPort, new_inport_id))

                            arg_value_id = self.id_manager.get_id("argumentValue")
                            self.graph.add(
                                (arg_value_id, rdflib.RDF.type, rdflib.Literal("sdtl#argumentValue")))

                            # Connect it to the port
                            self.graph.add(
                                (new_inport_id, self.id_manager.sdtl_namespace.argumemtValue, arg_value_id))

                            for arg_value_key in arg["argumentValue"].keys():
                                source_info = f'sdtl#{arg_value_key}'
                                self.graph.add(
                                    (arg_value_id, rdflib.URIRef(source_info), rdflib.Literal(arg["argumentValue"][arg_value_key])))
                            pass
    ...
    def parse_drop_command(self, command):
        ...
        if command['consumesDataframe']:
            ...
            for data_frame in command["consumesDataframe"]:
                in_port_id = self.id_manager.get_id("Port")
                self.graph.add((in_port_id, rdflib.RDF.type, self.id_manager.provone_ns.Port))
                self.graph.add((in_port_id, rdflib.RDF.type, self.id_manager.sdtl_namespace.Dataframe))
